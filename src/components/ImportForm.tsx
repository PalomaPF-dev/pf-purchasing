...
const KIND_INFO: Record<Kind, { label: string; note: string }> = {
  prices: {
    label: "単価申請の一括取込",
    note: "1ファイル＝1申請（下書き）として取り込みます。旧単価は入力不要で、取込時に単価履歴（適用日時点の単価）から自動で入ります。品名・単位・ロット数も履歴の登録内容で補完します。取込後、内容を確認して提出（承認へ回す）してください。",
  },
  items: {
    label: "品番マスタの一括登録",
  ...
  locations: {
    label: "納入場所マスタの一括登録",
    note: "mcframe の「場所マスタ」（場所ＣＤ／場所名／更新ユーザ名／更新日時）をそのまま取り込めます。場所ＣＤ→納入場所CD、場所名→納入場所名として登録し、同じCDの既存データは名称を上書きします。「論理削除」列があれば、論理削除された場所は「無効」として登録します（単価履歴の名称表示には使われます）。単価情報エクスポート（納入場所CD・納入場所名の列）も取込可。",
  },
  "supplier-contacts": {
    label: "取引先の担当窓口",
