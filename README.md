...
- **一括取込** — Excel/CSV から単価申請の明細・品番マスタ・取引先マスタ・取引先の担当窓口・社員マスタを一括登録（テンプレートDL付き。担当窓口と社員一覧は既存Excelをシート名で自動判別して取込可）
...
3. **社員マスタ**を一括取込（`/import?tab=employees`）→ `/employees` で「社員マスタからユーザーを登録 / 更新」。ログインユーザーと承認W/Fの承認者がまとめて作られます
4. **品番マスタ・取引先マスタ**を一括取込（`/import` → テンプレートDL → 取込）、続けて**取引先の担当窓口**（`/import?tab=supplier-contacts`）を取込んで担当バイヤー・チェイサーを割当
5. **データ移行**（`/migrate`）: mcframe の単価情報エクスポート（Excel）を「CSV UTF-8」で保存し、そのままアップロード
