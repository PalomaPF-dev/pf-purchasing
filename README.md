...
- **品番マスタ** — 品目CD・品名・単位ＣＤ・科目・科目名・科目内訳・借方ICS科目名（製造）・品目区分・材料区分を管理。追加・編集・削除に加え、7万行規模のExcelを数秒で一括登録（同一品目CDは品目区分を「3/7」形式でまとめる）。**現在の取引先**は単価履歴の最新適用単価から自動表示
...
- **データ移行** — mcframe からエクスポートした単価情報（21万行規模）をブラウザから分割送信で取込（冪等・再実行可）。**単価改訂履歴（理由つき）**を後から反映して、移行済み履歴に改訂理由・内訳・担当者を補完
