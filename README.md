...
- **担当発注先による制限** — 一般権限は取引先マスタで自分が担当に設定された発注先のみ、申請・閲覧できる（管理者は全件）。担当は取引先ごとに**企画グループ（バイヤー主担当・副担当）＋管理グループ（チェイサー）**の3枠
- **社員マスタ** — 社員番号・氏名・承認W/F（MGR/部門長）・権限を管理し、**ワンクリックでログインユーザーと承認者を一括登録**。担当窓口取込の氏名→社員番号の名寄せにも使用
...
- **一括登録** — Excel/CSV の一括取込は各マスタ画面に内蔵（テンプレートDL付き。担当窓口と社員一覧は既存Excelをシート名で自動判別して取込可）
...
3. **設定 → ユーザー登録**（`/employees`）で社員一覧のExcelを一括登録 →「社員マスタからユーザーを登録 / 更新」。ログインユーザーと承認W/Fの承認者がまとめて作られます
4. **設定 → 取引先マスタ / 品番マスタ**の各画面から一括登録。取引先マスタでは続けて「取引先の担当窓口」を取込むと担当バイヤー・チェイサーが割り当たります
